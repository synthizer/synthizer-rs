    ...
    ($t: ty, $syz_const: expr, $getter: ident, $setter: ident) => {
        impl $t {
            pub fn $getter(&self) -> Result<bool> {
                ...
                check_error(unsafe { syz_getI(&mut out as *mut i32, self.to_handle(), $syz_const as i32) })?;
                Ok(out != 0)
            }
    ...
    ($t: ty, $syz_const: expr, $getter: ident, $setter: ident) => {
        impl $t {
            pub fn $getter(&self) -> Result<f64> {
                let mut out = Default::default();
                check_error(unsafe { syz_getD(&mut out as *mut f64, self.to_handle(), $syz_const as i32) })?;
                Ok(out)
            }
...
double_p!(BufferGenerator, SYZ_P_PLAYBACK_POSITION, get_playback_position, set_playback_position);
